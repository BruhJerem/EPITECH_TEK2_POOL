...
{
        char *s = malloc(sizeof(char) * (strlen(tab[0])+2));
	char *delim_str = strdup(" \0");

        delim_str[0]=delim;
        for (int i=0; tab[i] != NULL; i++) {
                if (!s)
                        s = realloc(s, sizeof(char)*(strlen(s)+strlen(tab[i])+2));
                if (i!=0)
	                s=strcat(s, delim_str);
                s=strcat(s, tab[i]);
	}
        clear(this);
        assign_c(this, s);
...
{
	char *s = malloc(sizeof(char) * (strlen(tab[0].str)+2));
	char *delim_str = strdup(" \0");

	delim_str[0]=delim;
	for (int i=0; tab[i].str != NULL; i++) {
		if (!s)
			s = realloc(s, sizeof(char)*(strlen(s)+strlen(tab[i].str)+2));
		if (i!=0)
			s=strcat(s, delim_str);
		s=strcat(s, tab[i].str);
	}
	clear(this);
	assign_c(this, s);
}
...
{
	char *s = malloc(sizeof(char)*(strlen(this->str)+1));
	if (offset < 0) {
		memcpy(s, this->str + (strlen(this->str) - (offset * (-1)) - length), length);
	} else if (length < 0) {
		memcpy(s, this->str + (strlen(this->str) - (offset*(-1))), length);
	} else {
		memcpy(s, this->str+offset-1, length);
	}
	clear(this);
	assign_c(this, s);
        return (this);
