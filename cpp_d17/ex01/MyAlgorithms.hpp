...
{
        return static_cast<size_t>(std::count(contener.begin(),
					      contener.end(), value));
}
...
{
        std::rotate(contener.begin(), contener.begin() + nbShift
		, contener.end());
}
...
template<typename T>
void vFusionOrderedLists(T const & contener1, T const & contener2,
			 T& contenerToFill)
{
        std::merge(contener1.begin(), contener1.end(),
		   contener2.begin(), contener2.end(), contenerToFill.begin());
}
